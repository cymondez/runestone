...
| 代價 | 對應約束 | 章節 |
| --- | --- | --- |
| 修改不屬於自己的檔案 | 預設關閉；唯一動作是在最前面插入一筆；不搬移、不去重、不改寫任何既有項目 | 7.1、9.1 |
...
| 全機依賴單一容器 | `restart: unless-stopped`；`stop` 不停 dns；`down` 先撤銷再拆除 | 8.2、10.2、10.4 |
...
- `runestone setup` 的 DNS 開關、上游 DNS 與自動排序設定，以及對應的風險揭露。
...
DNS_UPSTREAM=           # 上游 DNS，逗號分隔，使用者可設
DNS_AUTO_REORDER=false  # 我方項目不在 dns[0] 時，是否自動把它排回第一筆（見 9.6）
...
- `phase`：`prepared`（已寫入 daemon 設定，尚未完成重啟與驗證）→ `applied`（已驗證成功）。
- `insertedEntry` 是**唯一**由 Runestone 擁有的項目。`value` 用於比對，`index` 是最後一次已知位置，**只用於識別，不用於還原位置** — 撤銷時只把該筆移除，不做任何位置還原動作。
...
1. `.env` 的 `DNS_UPSTREAM`（使用者明確設定，優先權最高）。
2. daemon 設定檔中既有的 `dns` 項目，排除 loopback 與 Target IP。
3. 主機解析器（Node `dns.getServers()`），排除 loopback 與 Target IP。
4. 都取不到 → `dns enable` 在修改 daemon 設定前中止，提示執行 `runestone setup` 設定上游。

不自動填公用 DNS（例如 1.1.1.1）：企業或校園網路可能只允許內部解析器。

**Runestone 不會為了備援而改動使用者原有的 daemon `dns` 項目順序或內容**，只在最前面插入 Target IP；備援由 dnsmasq 的上游負責。
...
Runestone 只做一個動作：**把 Target IP 插入 `dns` 陣列的 index 0**。不搬移、不去重、不改寫任何既有項目。

| 現況 | 動作 | 記錄 |
| --- | --- | --- |
| 沒有 `dns` key | 建立 `dns: [TargetIP]` | `createdDnsKey=true`、`insertedEntry={value, index: 0}` |
| 已有 `dns` 陣列 | 在 index 0 插入 Target IP；既有項目整體往後一位，內容與相對順序不變 | `insertedEntry={value, index: 0}` |
...
2. 依 9.5 的識別順序找出 `insertedEntry`，**只移除那一筆**。其餘項目一律原位保留，不做任何位置還原或重排。
...
原則：**Runestone 只認自己插入的那一筆。其餘一切都屬於使用者，包含使用者在啟用之後才新增或修改的項目。**

我方項目的識別順序：

1. 記錄的 `index` 位置上的值與 `value` 相符 → 即為我方項目。
2. 否則，陣列中值相符的項目**只有一筆** → 即為我方項目。
...
| `DNS_AUTO_REORDER` | `up` / `dns enable` 偵測到我方項目不在 `dns[0]` 時 |
| --- | --- |
| `false`（預設） | **只警告，不修改任何內容。** 警告需說明：我方項目目前的實際位置、DNS 可能不會生效、以及可設 `DNS_AUTO_REORDER=true` 讓 Runestone 每次啟動時自動排回第一筆。 |
| `true` | 只把**我方記錄的那一筆**搬到 index 0，其他項目的相對順序完全不變，更新記錄的 `index`，並告知使用者「daemon 設定已更新，將於下次 Docker 重啟後生效」。 |

兩種模式的共同限制：

- **絕不搬移、修改或移除任何非我方的項目。** 自動排序調整的只有我方那一筆的位置。
...
1. **前置檢查（不改動任何狀態）**：setup 已完成、Docker 可用、context 非 remote、非 Windows container 模式、可解析出 IPv4 Target IP、daemon 設定檔可讀且為合法 JSON、可取得上游 DNS。
...
唯讀。輸出：期望狀態、dns 服務狀態、Target IP、Bind IP、daemon 路徑、所有權記錄（含 `phase`）、上游 DNS、UI 網址與驗證狀態。

外部變更偵測結果也必須顯示（依 9.5）：

- 我方項目是否仍存在、值是否仍相符。
- 我方項目是否仍為 `dns[0]`；若不是，顯示目前實際位置、「DNS 可能不會生效」的提示，以及目前 `DNS_AUTO_REORDER` 的設定值。
...
DNS 相關共三題，順序固定，第二三題僅在啟用時出現：

| 題目 | 預設 | description 必須說明 |
| --- | --- | --- |
| 是否啟用 DNS | 關閉 | 11.3 的第 1、2、3、4、7 項，並帶入實際的 daemon 路徑 |
| 上游 DNS | 空（走 8.4 的自動偵測） | 用途、留空時的偵測順序、以及偵測不到會導致啟用中止 |
| 是否啟用自動排序 | 關閉 | 兩種模式各自的風險：關閉時使用者插隊後 DNS 可能不生效且 Runestone 不會自動修正；開啟時 Runestone 會在每次 `up` 改寫 daemon 設定（只改我方那筆、不重啟 Docker） |
...
| 7 | 停用時只移除 Runestone 自己加入的那一筆，其他項目與順序完全不動 |
| 8 | dns UI 在未設定 `DNS_UI_USER` / `DNS_UI_PASS` 時無任何驗證 |
| 9 | 自動排序開啟時，Runestone 會在每次 `up` 改寫 daemon 設定（只改我方那筆，不重啟 Docker） |
...
| `dns enable` 修改 daemon 之前的確認 | 1–9 全部，帶入實際值 |
| `dns enable` 重啟 Docker 之前的確認 | 2，明說會終止全機 container |
| `dns status` | 目前生效中的侵入性設定：daemon 路徑、我方項目實際位置、Bind IP、UI 驗證狀態、自動排序設定 |
...
| README / DESIGN 文件 | 第 1–8 項的摘要與手動解除步驟 |
...
| 取不到上游 DNS | 中止 | 設定 `DNS_UPSTREAM` 後可 |
...
| 我方項目已不是 `dns[0]` | 依 `DNS_AUTO_REORDER`：`false` 只警告；`true` 只搬移我方那筆並告知下次 Docker 重啟後生效 | 可 |
...
- `DNS_AUTO_REORDER` 兩種模式：`false` 只警告且檔案零變更；`true` 只搬移我方那筆、其他項目相對順序不變、記錄的 `index` 有更新、且不觸發 Docker 重啟。
- 再入性：重複執行 `up` / `dns enable` 不得插入第二筆我方項目。
- 寫檔保真：原檔縮排風格與其他 key 的順序在寫回後不變。
- 憑證 domain 集合的變化偵測：集合相同時不重啟 `dns` 服務，集合變化時才重啟。
- 上游 DNS 決定順序與 loopback / Target IP 排除。
- Bind IP 平台判定。
...
- setup 的三個 DNS 題目：預設值（停用、上游留空、自動排序關閉）、錯誤回到同一題、review 摘要顯示三個值、取消不留痕跡。
- 風險揭露（11.3 揭露時機表的每一列都要有測試）：對應的揭露項目確實出現，且帶入實際的 daemon 路徑、Target IP、Bind IP 而非佔位字串；`--yes` 仍然輸出揭露內容。
- `stop` 不會停掉 dns 服務；`down` 在 disable 失敗時中止。
- `certs create` / `remove` 觸發 mapping 重建。

### 14.3 手動平台驗證

Windows Docker Desktop、WSL2、macOS（Intel 與 Apple Silicon）、Linux rootful。每個平台驗證：啟用 → 新 container 的 resolv.conf 首筆為 Target IP → 解析憑證涵蓋的子網域得到 Target IP → 停用後 daemon `dns` 陣列回到原狀（含使用者原有項目）。
...
2. **dns image 的建置與發佈方式**：`make/docker.mk` 指向的 `docker-bake.hcl` 不在 repo 內（fork 時就缺了）。dns image 要補一份 bake 檔一起管、還是用獨立的 buildx 指令發佈？另外 image 名稱與起始 tag 請確認（暫定 `cymondez/runestone-dns:1.0`）。
3. **`TODO` 寫的「必須讓使用者設定其他 dns 作為備援」**：本規格把備援放在 dnsmasq 上游（`DNS_UPSTREAM`），不寫進 daemon `dns` 陣列；理由是寫進去就得一併記錄與撤銷，且 `stop` 保留 dns 服務已能避免全機 DNS 中斷。若你要的是「daemon 陣列裡也放一組備援 IP」，需要把 `insertedEntry` 改成陣列（見 7.3、9.1）。
