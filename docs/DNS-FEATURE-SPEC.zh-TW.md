...
- **IPv6 全部不在範圍內，而且這是整個套件的範圍決策，不只是 DNS 功能的。** 理由是 IPv4 基本上到處都支援、行為一致，IPv6 則不保證存在（取決於主機、網路，以及 Docker 自己有沒有開），支援它等於要支援那整個矩陣；為了避開這類複雜度，套件只針對 IPv4 場景。這個決策在 DNS 功能裡的樣子是：Target IP 以 `getent ahostsv4` 解析、daemon 項目與綁定位址與發佈的埠都是 IPv4、`address=/domain/<ipv4>` 只回答 A（其餘型別為 NODATA 且不轉發，已逐型別量測）。而在預設安裝下 container 本來就沒有 IPv6（`bridge` 的 `EnableIPv6=false`，量測確認 container 的 eth0 沒有任何 inet6 位址），所以 AAAA 的答案也無路可走。開啟 Docker IPv6 之後的行為**從未量測**——詳見 [`DOMAINS.zh-TW.md`](DOMAINS.zh-TW.md)。
...
- 低於門檻時 Runestone 不做任何嘗試，並給出三條路：更新 Docker Desktop、透過它自己的設定重啟、或 `dns enable --restore-containers`——那會記下執行中的容器，並把重啟後沒回來的啟動回去。**最後那一條是選用的、永遠不是預設**，因為「啟動不屬於 Runestone 的容器」不是它可以自己決定的事。
...
**setup 只記錄設定，不負責啟用 DNS。** 它會寫入 `DNS_UPSTREAM`、`DNS_DAEMON_FALLBACK` 與 `DNS_AUTO_REORDER`，執行 10.1 步驟 1 的唯讀前置檢查（讓做不到的機器當場就說出來），然後引導使用者去執行 `runestone dns enable`。三個理由，而第三個才是定案的那個：

- **setup 在任何方向上都不會寫 `DNS_ENABLE`。** 寫 `true` 等於宣稱 DNS 已開啟，但 daemon 設定裡什麼都沒有——那正是 `up` 與 `doctor` 存在要警告的狀態；寫 `false` 則會安靜地讓一筆已經在檔案裡的項目變成孤兒。那個鍵屬於 `dns enable` 與 `dns disable`，因為它背後那個檔案是它們擁有的。
- 11.3 那兩道強制確認——寫入前一次、重啟前再一次——屬於使用者為了做這件事而親自下的那個指令，不屬於他為了別的目的跑的精靈的結尾。
- **啟用流程需要一個 setup 還沒建立出來的環境**：`applyEnable` 會啟動 dns 服務，而 Compose 專案的網路是由 `up` 建立的。從 setup 裡面啟用，第二次跑會成功、第一次跑會失敗。

因此上面那條限制是「按構造成立」而不是「靠回滾成立」：setup 不寫 daemon 設定、不建立服務、也不記錄任何所有權狀態，所以取消時根本沒有東西需要還原。
...
呈現方式：`dns enable` 的揭露以左欄標籤分組呈現，不再逐條編號。第 1–11 項的事實一項不減、值一律帶入，但每一行都控制在 80 欄以內（daemon 路徑與 UI 網址例外，它們是值，不能縮短，因此各自獨佔一行），且只有三行標為警告：重啟、依賴、未設驗證的 UI。編號並列會讓十一件事處在同一個視覺層級，八行超過 80 欄後由終端機自行斷行，縮排也就散了——十三行印出來實際佔二十一行。每一行背後的理由移到使用者說明文件。
...
機械性的部分由 `docker/dns/test/m6b-safety-net.sh` 處理：事前 `capture`、任何時候 `status`、被留在沒人預期的狀態時 `restore`、結束後 `clean`。它會快照 daemon 檔與其雜湊、Runestone 的工具狀態，以及每個執行中的容器**連同它的 restart policy**——policy 正是「daemon 重啟後這個容器會不會自己回來、還是得有人再叫它一次」的決定因素。

- 把現在的 `daemon.json` 複製到版控之外並記下雜湊值。這是給人用的安全網，不是 Runestone 管理的備份——9.3 禁止整檔還原的規則不變。**那個腳本會整檔還原，而 CLI 永遠不會呼叫它**：它由排定這個時間窗的人手動執行，對象是幾分鐘前才拍下的快照，而且會在丟棄之前先把即將丟棄的 diff 印出來。
...
- **只為「Docker 真正會讀的那個檔」重啟 Docker。** 腳本會自己推導那條路徑，被 `RUNESTONE_DNS_DAEMON_PATH` 指到別處時就拒絕重啟。這道防護存在的理由是：這個腳本曾在自己的測試過程中，對著一個暫存檔操作卻重啟了一台正在工作的機器——一個會中斷它所保護的機器的安全網，不算安全網。
