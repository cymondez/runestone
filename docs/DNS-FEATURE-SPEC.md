...
Each decision below has a milestone deadline; see 16.6. Decision 3 in particular must be settled before M1 begins.
...
2. **How the runestone-dns image is built and published**: it needs a multi-arch (`linux/amd64` + `linux/arm64`) build and publish path, and the repository currently has none that this feature can rely on. Decide what that path is — a `docker buildx build --platform` invocation kept with the image, a CI workflow, or something else — and confirm the image name and initial tag (currently assumed `cymondez/runestone-dns:1.0`). M2 cannot complete without this.
...
| 15.2 — how the image is built and published, its name and initial tag | Before M2 | — |
| 15.4 — the `compose.yml` regeneration strategy | Before M3 | — |
