...
| --- | --- | --- |
| Modifying a file we do not own | Disabled by default; the only action is inserting one entry at the front; never move, deduplicate or rewrite any existing item | 7.1, 9.1 |
...
| The whole machine depends on one container | `restart: unless-stopped`; `stop` leaves dns running; `down` revokes before tearing down | 8.2, 10.2, 10.4 |
...
- A DNS toggle, upstream DNS and automatic-reordering setting in `runestone setup`, together with the matching risk disclosures.
...
DNS_UPSTREAM=           # Upstream DNS, comma separated, user configurable
DNS_AUTO_REORDER=false  # Move our entry back to dns[0] when it is not first (see 9.6)
...
```json
{
  "dns": {
    ...
    "insertedEntry": { "value": "192.168.65.254", "index": 0 },
    "createdDnsKey": true,
...
- `phase`: `prepared` (daemon configuration written, restart and verification not yet complete) → `applied` (verified successfully).
- `insertedEntry` is the **only** item Runestone owns. `value` is used for matching and `index` is its last known position, used **for identification only, never to restore a position** — revocation removes that item and performs no positional restoration.
...
1. `DNS_UPSTREAM` from `.env` (explicit user configuration, highest priority).
2. Existing `dns` entries in the daemon configuration file, excluding loopback addresses and the Target IP.
3. The host's resolvers (Node `dns.getServers()`), excluding loopback addresses and the Target IP.
4. If none can be determined, `dns enable` aborts before modifying the daemon configuration and tells the user to configure an upstream through `runestone setup`.

Public resolvers (such as 1.1.1.1) are never filled in automatically: corporate and campus networks may only permit internal resolvers.

**Runestone never reorders or alters the user's existing daemon `dns` entries for the sake of a fallback** — it only inserts the Target IP at the front. Fallback behaviour is dnsmasq's upstream responsibility.
...
Runestone performs exactly one action: **insert the Target IP at index 0 of the `dns` array.** It never moves, deduplicates or rewrites any existing item.

| Current state | Action | Record |
| --- | --- | --- |
| No `dns` key | Create `dns: [TargetIP]` | `createdDnsKey=true`, `insertedEntry={value, index: 0}` |
| A `dns` array exists | Insert the Target IP at index 0; existing items shift down by one with their contents and relative order unchanged | `insertedEntry={value, index: 0}` |
...
2. Identify `insertedEntry` using the order defined in 9.5 and **remove only that one item**. Every other item stays exactly where it is; no positional restoration or reordering is performed.
...
Principle: **Runestone recognises only the single entry it inserted. Everything else belongs to the user, including entries the user added or changed after enabling.**

Order used to identify our entry:

1. The value at the recorded `index` still matches `value` → that is our entry.
2. Otherwise, if **exactly one** item in the array matches the value → that is our entry.
...
| Changed the value of our entry (e.g. replaced it with a different IP) | Ownership conflict: abort, print the daemon path, require `runestone dns disable --assume-entry <ip>` |
...
| `DNS_AUTO_REORDER` | When `up` / `dns enable` detects our entry is not at `dns[0]` |
| --- | --- |
| `false` (default) | **Warn only; change nothing.** The warning must state our entry's actual current position, that DNS may not take effect, and that `DNS_AUTO_REORDER=true` makes Runestone move its entry back to the front on every start. |
| `true` | Move **only the entry we recorded** to index 0, leaving the relative order of all other items untouched, update the recorded `index`, and tell the user that "the daemon configuration was updated and will take effect after the next Docker restart". |

Constraints shared by both modes:

- **Never move, modify or remove any item that is not ours.** Automatic reordering adjusts the position of our own entry only.
...
1. **Preflight, changing nothing**: setup completed, Docker available, context not remote, not Windows container mode, an IPv4 Target IP resolvable, the daemon configuration file readable and valid JSON, and upstream DNS obtainable.
...
5. After user confirmation, restart Docker and poll until it responds.
...
Read-only. Reports: desired state, dns service state, Target IP, Bind IP, daemon path, ownership record (including `phase`), upstream DNS, UI URL and authentication state.

External-change detection must also be shown (per 9.5):

- Whether our entry is still present and its value still matches.
- Whether our entry is still `dns[0]`; if not, show its actual current position, a "DNS may not take effect" notice, and the current `DNS_AUTO_REORDER` setting.
...
```text
runestone dns enable  [--yes] [--upstream <ip,...>]
runestone dns disable [--yes] [--assume-entry <ip>] [--assume-index <n>]
...
Three DNS questions in a fixed order; the second and third appear only when DNS is enabled:
...
| Upstream DNS | Empty (uses the automatic detection in 8.4) | What it is for, the detection order when left empty, and that a failed detection aborts enabling |
| Enable automatic reordering? | Disabled | The risk of each mode: when off, DNS may stop taking effect after the user inserts their own entry and Runestone will not correct it; when on, Runestone rewrites the daemon configuration on every `up` (its own entry only, without restarting Docker) |
...
- The review summary must show all three values.
...
| 7 | Disabling removes only the single entry Runestone added; all other entries and their order are untouched |
| 8 | The dns UI has no authentication unless `DNS_UI_USER` / `DNS_UI_PASS` are set |
| 9 | With automatic reordering on, Runestone rewrites the daemon configuration on every `up` (its own entry only, without restarting Docker) |
...
| Confirmation before `dns enable` modifies the daemon | All of 1–9, with real values |
| Confirmation before `dns enable` restarts Docker | 2, stating explicitly that every container will be terminated |
| `dns status` | The invasive settings currently in effect: daemon path, our entry's actual position, Bind IP, UI authentication state, automatic-reordering setting |
...
| README / DESIGN documents | A summary of items 1–8 and the manual removal steps |
...
| No upstream DNS obtainable | Abort | After setting `DNS_UPSTREAM` |
...
| The user replaced the value of our entry | Ownership conflict; abort and suggest `--assume-entry` | After manual recovery |
...
| Our entry is no longer `dns[0]` | Per `DNS_AUTO_REORDER`: `false` warns only; `true` moves our entry alone and reports that it takes effect after the next Docker restart | Yes |
...
- **`compose.yml` needs versioned regeneration**: `ensureProjectFiles()` currently writes the Compose file only when it is absent, so existing users who upgrade the CLI would never receive the new Compose file containing the dns service. A template version marker is needed (for example `COMPOSE_TEMPLATE_VERSION` in `.env`), regenerating when the version differs.
...
- Both `DNS_AUTO_REORDER` modes: `false` warns with zero file changes; `true` moves only our entry, leaves the relative order of other items unchanged, updates the recorded `index`, and never triggers a Docker restart.
...
- Upstream DNS resolution order, and exclusion of loopback addresses and the Target IP.
...
- The three setup DNS questions: defaults (disabled, empty upstream, reordering off), errors return to the same prompt, the review summary shows all three values, and cancelling leaves no trace.
- Risk disclosure (every row of the 11.3 timing table needs a test): the matching items actually appear, carrying the real daemon path, Target IP and Bind IP rather than placeholders; `--yes` still prints the disclosures.
...
Windows Docker Desktop, WSL2, macOS (Intel and Apple Silicon), Linux rootful. On each platform verify: enable → a new container's resolv.conf lists the Target IP first → a subdomain covered by a certificate resolves to the Target IP → after disabling, the daemon `dns` array is back to its original state, including the user's pre-existing entries.
...
2. **How the dns image is built and published**: the `docker-bake.hcl` referenced by `make/docker.mk` is not in the repository (it was already missing at fork time). Should the dns image be added to a restored bake file, or published with standalone buildx commands? Please also confirm the image name and initial tag (currently assumed `cymondez/runestone-dns:1.0`).
3. **The `TODO` requirement "users must be able to configure another DNS as fallback"**: this specification places the fallback in dnsmasq's upstream (`DNS_UPSTREAM`) rather than in the daemon `dns` array, because entries written there must also be recorded and revoked, and because `stop` keeping the dns service alive already prevents machine-wide DNS outages. If what you want is a fallback IP inside the daemon array as well, `insertedEntry` would need to become an array (see 7.3, 9.1).
4. **The `compose.yml` regeneration strategy**: regenerate automatically after an upgrade, or prompt the user to run `runestone setup`?
