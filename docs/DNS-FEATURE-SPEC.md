...
2. **Settled (revised 2026-08-30) — the image is published by a maintainer running one command that is written down, with no script and no CI.** The command is `docker buildx build --platform linux/amd64,linux/arm64 --push`, publishing `cymondez/runestone-dns:<tag>`, and it lives in `docker/dns/README.md`.

   The earlier answer was "a committed `publish.sh`, with CI deferred rather than abandoned". Both premises fell: **CI is now decided against**, so "interim" had no end; and the script was POSIX shell, which cannot run on the platform this project is mostly developed on — a wrapper nobody can run is worse than a command written down. The script is removed.

   **Traceability now rests on three things, all of them held by hand:** publish only from a clean `main` checkout; put that main commit in `org.opencontainers.image.revision`; and always name the tag rather than defaulting to `latest`.

   **State the debt plainly, because it is now permanent:** with no automated gate, whether a published image's recorded origin is true depends entirely on whether the person publishing followed this. A revision label pointing at a feature-branch commit, or at one that exists on a single machine, is a lie that nothing afterwards can detect.
